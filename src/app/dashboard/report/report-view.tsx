...
export type ReportRange = { key: string; label: string; days: number };
...
export function parseReportRange(v: string | undefined): ReportRange {
  return REPORT_RANGES.find((r) => r.key === v) ?? REPORT_RANGES[2];
}
...
/**
 * Shared report body. Used by the authed /dashboard/report and the public
 * tokenized /r/[token] link. `basePath` drives the range-switcher hrefs;
 * `publicView` hides internal-only affordances.
 */
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="text-[10.5px] uppercase tracking-[0.18em] font-semibold text-[var(--color-fg-muted)]">
            {publicView ? "EscapeHatch · Live test readout" : "Client report"}
          </div>
        ...
        <RangeLinks active={range.key} basePath={basePath} />
      </div>
      ...
      <section className={`rounded-lg border p-5 ${validityClass(validity.level)}`}>
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            ...
            <div className="mt-2 text-[20px] md:text-[24px] font-semibold tracking-tight">{validity.headline}</div>
            <div className="mt-1.5 text-[12.5px] text-[var(--color-fg-dim)] max-w-3xl">
              Directional numbers stay labeled directional until traffic, purchase signal, sample size, and
              confidence are strong enough to call the test.
            </div>
          </div>
          ...
          <div className="px-4 py-3 space-y-2">
            {validity.checks.map((check) => (
              <div key={check.key} className="flex items-start gap-2 rounded-md border border-[var(--color-border-soft)] px-3 py-2">
                <PixelIcon name={check.passed ? "check" : "clock"} size={13} className={check.passed ? "mt-0.5 text-[var(--color-success)]" : "mt-0.5 text-[var(--color-fg-muted)]"} />
                <div className="min-w-0">
      ...
      <div className="grid gap-4 lg:grid-cols-12">
        ...
        <section className="lg:col-span-5 rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-card)]">
          <Header title="Read this carefully" sub="Keep this visible until the test is ready." />
          <div className="px-4 py-3 space-y-3 text-[12.5px] text-[var(--color-fg-dim)] leading-relaxed">
            ...
            <p>Treat the headline as directional until the validity panel says ready. Early tests can swing from one or two high-AOV orders.</p>
            <p>The safest phrasing is lift in revenue per visitor, not raw recovered revenue, until the test is fully powered.</p>
          </div>
        </section>
      </div>

      {publicView ? (
        <div className="text-center text-[11px] font-mono text-[var(--color-fg-muted)]">
          Live data · updates continuously · powered by EscapeHatch
        </div>
      ) : (
            ...
            <div>
              ...
              <div className="mt-1 text-[12.5px] text-[var(--color-fg-dim)]">
                {validity.level === "ready"
                  ? "Share the public report link with the merchant."
...
function RangeLinks({ active, basePath }: { active: string; basePath: string }) {
  return (
    <div className="inline-flex rounded-md border border-[var(--color-border-soft)] bg-[var(--color-card)] p-[2px]">
      {REPORT_RANGES.map((r) => (
        <Link
          key={r.key}
          href={r.key === "30d" ? basePath : `${basePath}?range=${r.key}`}
        ...
        return (
          <div key={row.label} className="py-3 first:pt-0 last:pb-0">
            <div className="flex items-baseline justify-between gap-3">
              ...
              <div className={`text-[12px] font-mono tnum ${toneText(lift)}`}>{idx === 0 ? "baseline" : fmtPct(lift)}</div>
            </div>
...
function Kpi({ label, value, sub, tone = "neutral" }: { label: string; value: string; sub: string; tone?: "good" | "bad" | "neutral" }) {
  return (
    <div className="rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-card)] px-4 py-3">
      ...
      <div className={`mt-1 text-[24px] font-semibold tracking-tight tnum ${tone === "good" ? "text-[var(--color-success)]" : tone === "bad" ? "text-[var(--color-danger)]" : ""}`}>{value}</div>
