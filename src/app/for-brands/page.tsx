...
// Unlisted sales landing page — sent directly to prospects, kept out of search.
...
export default function ForBrands() {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[var(--color-bg)] text-[var(--color-fg)]">
      ...
      {/* Hero */}
      <section className="relative z-10 mx-auto max-w-4xl px-6 pt-16 pb-14 text-center sm:pt-24">
        ...
        <div className="hero-enter hero-enter-5 mx-auto mt-14 grid max-w-2xl grid-cols-1 gap-3 sm:grid-cols-3">
          <Stat value="+8–56%" label="RPV lift, by brand" sub="all-time, vs. a live control" />
          <Stat value="240k+" label="shoppers measured" sub="across 7 brands" />
          <Stat value="~15 min" label="to install" sub="no heavy dev work" />
        </div>

        {/* Brand showcase */}
        <div className="hero-enter hero-enter-6 mx-auto mt-6 max-w-3xl">
          <BrandShowcase />
        </div>
      </section>
      ...
      {/* Proof — named brands */}
      <Section id="proof" eyebrow="The proof" title="Real brands. Real lift. Measured against a live control.">
        <p className="mx-auto mb-9 max-w-2xl text-center text-[15.5px] leading-relaxed text-[var(--color-fg-dim)]">
          Every install runs as a 50/50 A/B test — half of your in-app-browser shoppers are escaped,
          half stay as a control. The lift below is the <span className="text-[var(--color-fg)]">measured</span>{" "}
          revenue-per-visitor difference between the two, all-time, on each brand&apos;s own store.
        </p>
        ...
        <div className="card-hi mx-auto mt-8 max-w-3xl px-6 py-5">
          <div className="eyebrow mb-2">Why this is different</div>
          <p className="text-[15px] leading-relaxed text-[var(--color-fg-dim)]">
            Most &ldquo;recovery&rdquo; tools show you an estimate. EscapeHatch keeps a real control
            group running, so the lift on your dashboard is the measured gap between escaped and
            not-escaped shoppers <span className="text-[var(--color-fg)]">on your own store</span>, with
            statistical significance attached (COVE&apos;s is p&nbsp;&lt;&nbsp;.001). If it&apos;s not
            working for you, you&apos;ll know — and so will we.
          </p>
        </div>
      </Section>
      ...
      <Section eyebrow="Onboarding" title="Live in about 15 minutes">
        <div className="card mx-auto max-w-2xl overflow-hidden p-2">
          <InstallStep n="1" title="Paste the snippet" body="One script tag in your theme. We give you the exact code, scoped to your store." />
          <InstallStep n="2" title="Add the Customer Events pixel" body="A custom pixel in Shopify → Settings → Customer events, so we can read your funnel." />
          <InstallStep n="3" title="Add the Order-paid webhook" body="A one-line webhook so purchase data flows in authoritatively. Then watch the holdout." last />
        </div>
...
function InstallStep({ n, title, body, last }: { n: string; title: string; body: string; last?: boolean }) {
  return (
    <div className={`flex items-start gap-4 px-5 py-4 ${last ? "" : "border-b border-[var(--color-border-soft)]"}`}>
...
function GhostIcon() {
  return (
    <svg viewBox="0 0 24 24" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.8">
      <path d="M5 20V11a7 7 0 0 1 14 0v9l-2.3-1.6L14.4 20 12 18.4 9.6 20 7.3 18.4 5 20Z" strokeLinejoin="round" />
