-- Admin performance should count attributed test purchases even when the
-- purchase row has a null iab_kind. Some webhook/cart-token joins preserve
-- the bucket but not the source kind, especially on the escaped browser side.
