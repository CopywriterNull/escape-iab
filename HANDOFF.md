...
> **2026-05-20 update (dashboard v3):** `/dashboard/v3` is a new client-board variant designed for screenshots. It keeps Claude's v2 data ideas (phase, cumulative incremental, source/platform context, holdout cost) but reorganizes the UI into one above-the-fold story: incremental revenue, decision state, A/B proof, GT2 RS benchmark, and compact caveats. Classic `/dashboard` and `/dashboard/v2` now link to v3; v3 links back to both.
